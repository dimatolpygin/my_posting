...
export function panelRouter() {
    ...
    try {
      ...
      res.type('html').send(
        page({
          ...
          user: req.user,
          heading: 'Обзор',
          sub: 'Сводка по системе. Разделы наполняются по мере работы системы.',
    ...
    try {
      ...
      res.type('html').send(
        page({
          ...
          user: req.user,
          heading: 'Настройки',
          sub: 'Значения засеяны из брифа. Часть ключей правится только в базе.',
          message: buildSourceMessage(req.query),
          body,
        }),
      );
    ...
    try {
      const body = `
        ...
        ${await promptCard('image_prompt', 'Промт обложки', 8,
          'Используется при генерации картинки.')}`;
  ...
  // отвечает за 30-90 секунд — длинные задачи уходят в фон.
  ...
  // секунды, человек в панели ждёт результат. Длинные задачи уходят в фон.
  ...
  // а физический DELETE её уносит (на уровне схемы это запрещено).
  ...
  router.post('/manual', async (req, res) => {
    try {
      ...
      // Группу не публикуем сами: правило — «показывает результат до публикации».
  ...
  /** Карточка активного задания: шаг, счётчики, слоты, кнопка «Остановить». */
  ...
  router.post('/run', async (req, res) => {
    try {
      ...
      // показывает до запуска, и к прогону по расписанию.
