 ...
 *   Добор передаёт более раннюю дату, чтобы дочерпать архив, не трогая настройку.
 * @param {Date} [options.until] верхняя граница периода. Только наполнение из архива
 *: без неё активный сайт отдаёт свои свежие материалы вместо архивных.
  ...
  try {
    return await withSourceLock(source.id, async () => {
      ...
      for (const [index, article] of pending.entries()) {
        ...
        // выглядят как перебор: vklader после такого залпа закрыл доступ
