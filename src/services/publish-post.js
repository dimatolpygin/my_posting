...
/**
 * Публикация поста в группы Одноклассников через postmypost.
 *
 ...
 * 2. **Состояние аккаунта проверяется до попытки.** Токен подключения истекает
 ...
 *    в панели врёт без учёта публикаций. Считаются успешные публикации за сегодня по МСК.
 *
 * Режим по умолчанию — `draft` (черновик, настройка `publish_mode`): в группу ничего
...
export async function publishPost(post, { groupIds, mode, postAt, ignoreDailyLimit = false } = {}) {
  ...
  // синхронизирована неделю назад, а доступ к группе отвалиться вчера.
  const chanelId = await settings.getInt('pmp_chanel_id', pmp.CHANEL_OK);
  const accounts = new Map(
    (await pmp.okAccounts(chanelId)).map((item) => [String(item.id), item]),
  );
  ...
  for (const group of targets) {
    ...
    try {
      ...
      if (Number(account.connection_status) !== pmp.CONNECTION_OK) {
        ...
        throw new Error(
          `Аккаунт группы «${group.name}» отключён в postmypost ` +
            ...
            'Переподключите группу ОК в postmypost и повторите.',
        );
      }
      ...
      // и лимит должен ограничивать группу в конкретный день, а не темп создания.
...
/** Первый готовый пост с обложкой → в группы. Кнопка в панели, дальше cron. */
