...
/**
 * Промты лежат файлами в репозитории, но рабочая копия — в БД: она правится в панели
 * без деплоя. Файл используется только как первичное значение на чистой базе; живую
 * базу новым текстом наполняет миграция (см. 0025 для текста поста и 0026 для обложки).
 */
const PROMPT_SEEDS = [
  {
    key: 'post_prompt',
  ...
  },
  {
    ...
    file: 'docs/prompts/image_prompt.seed.txt',
  },
];
...
async function seedPrompts() {
  for (const seed of PROMPT_SEEDS) {
    ...
    // Зеркало в settings — часть кода читает промт оттуда.
    ...
    // развёртывание с нуля дало пустой раздел «Промты» и
