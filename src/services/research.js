...
/**
 * Сбор материала по поисковой фразе — для тем из очереди ключей.
 *
 * Отдельная функция, а не режим предыдущей, потому что задача другая. Там на входе
 * название проекта, и главная работа — отсеять страницы не про него. Здесь на входе
 * сам поисковый запрос, по которому мы и собираемся ранжироваться: выдача по нему
 * релевантна по определению, отсеивать не надо, а надо прочитать, что там уже написано.
 *
 * Зачем вообще: без этого шага модель пишет по теме «как оплатить зарубежный сервис»
 * то, что считает правдоподобным, а не то, что правда. Интернета у неё нет, проверить
 * себя нечем, и получается уверенный текст без единого проверяемого факта. Наши
 * карточки закрывают только то, что мы делали руками; всё остальное должно приехать
 * из чужих статей — с прямым запретом переписывать их формулировки.
 *
 * Расход: **один вызов firecrawl на пост**, но лимит считается по числу результатов.
 * При десяти темах в день и трёх страницах это 900 результатов в месяц — вплотную
 * к бесплатной тысяче, поэтому число страниц вынесено в настройку.
 *
 * @param {object} article строка articles
 * @param {{phrase?: string}} [options] поисковая фраза; по умолчанию — тема материала
 * @returns {Promise<{text: string, urls: string[], pages: number}|null>}
 */
export async function collectForKeyword(article, { phrase } = {}) {
  if ((await settings.get('research_keyword_mode', 'on')) === 'off') return null;
  if (!firecrawl.isConfigured()) {
    logger.warn('FIRECRAWL_API_KEY не задан — тема будет написана без чужих материалов');
    return null;
  }

  const query = String(phrase || article?.topic_name || article?.title || '').trim();
  if (!query) return null;

  const limit = await settings.getInt('research_keyword_results', 3);
  const perPage = await settings.getInt('research_chars_per_page', 3000);
  ...
  try {
    ...
    for (const page of found) {
      ...
      // Единственный фильтр — на пустышку: страница-навигация без содержания.
      // Фильтра «про тему» здесь нет намеренно: тема — это и есть запрос.
    ...
    if (pages.length === 0) {
      logger.warn(
        { материал: article.id, запрос: query },
        `По запросу «${query}» пригодных страниц нет — тема пишется по своей фактуре`,
      );
      return null;
    }

    const text = buildKeywordMaterial(query, pages).slice(0, MAX_TOTAL_CHARS);
    const urls = pages.map((page) => page.url);
    await articles.saveResearch(article.id, { text, urls });

    logger.info(
      { материал: article.id, запрос: query, страниц: pages.length, символов: text.length },
      `Прочитано по запросу «${query}»: ${pages.length} страниц, ${text.length} символов`,
    );
    return { text, urls, pages: pages.length };
  } catch (error) {
    // Сбор никогда не роняет генерацию: без материала тема получится беднее,
    // но получится. Причина остаётся в журнале «Ошибки».
    logger.error(
      { материал: article.id, запрос: query, ...errFields(error) },
      `Чтение чужих статей по «${query}» не удалось — тема пишется по своей фактуре`,
    );
    await captureError('чтение по теме', error, {
      service: 'firecrawl',
      articleId: article.id,
      details: `запрос: ${query}`,
    });
...
/**
 * Склейка прочитанного для промта.
 *
 * Шапка здесь важнее самого текста. Без неё модель принимает выдержки за образец
 * и пересказывает чужую статью близко к тексту — а она уже в выдаче, и наша копия
 * встанет позади неё, если встанет вообще. Поэтому материал прямо назван чужим
 * и снабжён запретами: цифры брать, формулировки нет.
 */
function buildKeywordMaterial(query, pages) {
  const parts = [
    `Ниже выдержки из чужих статей, найденные по запросу «${query}».`,
    'Это материал для проверки фактов, а не образец для пересказа. Правила работы с ним:',
    '',
    '- Цифры, названия сервисов, ограничения и сроки бери отсюда, а не из головы.',
    '  Чего в материале нет - того не пиши: выдуманная конкретика хуже её отсутствия.',
    '- Формулировки не повторяй. Эти статьи уже стоят в выдаче, и пересказ близко',
    '  к тексту встанет позади них.',
    '- Названия конкретных компаний-конкурентов, их цены и адреса сайтов в пост',
    '  не переноси.',
    '- Источники противоречат друг другу - бери осторожную формулировку или пропусти',
    '  факт совсем.',
    '- Свой опыт из карточек фактуры важнее: он есть только у нас, и ради него тему',
    '  и читают. Чужой материал закрывает то, чего в карточках нет.',
  ];
