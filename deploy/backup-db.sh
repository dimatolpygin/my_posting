...
# Ставится в cron скриптом scripts/install.sh (/etc/cron.d/my-posting-backup).
set -Eeuo pipefail

cd "$(dirname "$0")/.."
BACKUP_DIR="${BACKUP_DIR:-/opt/projects/my-posting/backups}"
...
FILE="$BACKUP_DIR/my-posting_$STAMP.sql.gz"
...
mv "$FILE.part" "$FILE"
find "$BACKUP_DIR" -name 'my-posting_*.sql.gz' -mtime +"$KEEP_DAYS" -delete
